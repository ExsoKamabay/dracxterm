 ...
 *   - an archive is available (asset or file)-> provision, then LINUX
...
class BootManager(private val ctx: Context) {
    ...
    fun boot(listener: Listener): Outcome {
        ...
        // 2. Discover an image: bundled in assets, or one the user obtained.
        ...
        if (archive == null) {
            ...
            return if (state.imageOfferDeclined) {
        ...
        return provision(archive, listener)
    }
    ...
    fun provision(archive: RootfsArchive, listener: Listener): Outcome {
        ...
        if (archive.source is RootfsArchive.Source.LocalFile) {
            listener.onStage(ProvisioningState.Stage.VERIFYING, "Reclaiming space…", 99)
