...
class RootfsValidator(private val ctx: Context) {
    ...
    fun validate(archive: RootfsArchive, targetDir: File): Result {
        Log.i(ShellLocator.TAG, "[VALIDATOR] Archive validation started: ${archive.fileName}")
        // 1. readable + at least a full header
        val magic = ByteArray(6)
        val read = runCatching {
            ctx.assets.open(archive.assetPath).use { ins ->
                var off = 0
        ...
        val compressed = runCatching { ctx.assets.openFd(archive.assetPath).use { it.length } }.getOrDefault(0L)
